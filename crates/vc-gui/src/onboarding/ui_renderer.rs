// Adapted from egui_kittest 0.34.3 (MIT OR Apache-2.0), emilk/egui.
// Test-only Vulkan renderer: upstream enables a DX12 allocator with incompatible
// Windows bindings. Keep the application on its existing glow renderer.
use std::sync::Arc;
...
use super::texture_to_image::texture_to_image;
...
/// Utility to render snapshots from a [`egui_kittest::Harness`] using [`egui_wgpu`].
...
impl egui_kittest::TestRenderer for WgpuTestRenderer {
    ...
    /// Render the [`egui_kittest::Harness`] and return the resulting image.
    fn render(
        &mut self,
        ctx: &egui::Context,
